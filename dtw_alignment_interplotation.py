"""
Specialized DTW alignment with explicit resampling and interpolation for nanopore traces.

This module resamples the normalized current traces prior to DTW so that the input
signals share a common length, then selects a representative reference trace and
computes a barycenter-like average aligned signal.
"""
...
def dtw_alignment_interplotation(
    ...
    """
    Aligns nanopore traces using DTW after resampling to a common length.

    Differences from dtw_alignment.py:
    - Adds an explicit resampling/interpolation step with TimeSeriesResampler before DTW.
    - Uses a common time axis for all traces by resampling to the maximum input length.
    - Produces aligned 2D arrays of time and current values and computes an average current trace.

    Args:
        time_normalized_data (List[pd.DataFrame]): List of normalized nanopore traces.
            Each DataFrame must contain time in the first column and current in the second.
        resample_length (Optional[int]): Target length for resampled aligned signals.
            If None, the maximum input trace length is used.
        plot_series (bool): If True, plots the resulting averaged current signal.
        plot_alignment (bool): If True, plots the DTW-aligned traces.
        show_matrix (bool): If True, displays a normalized pairwise DTW distance matrix.
        constraint (Optional[str]): DTW constraint type such as 'sakoe_chiba'.
            If None, unconstrained DTW is used.

    Returns:
        Tuple[List[np.ndarray], np.ndarray]:
            - List[np.ndarray]: A list of aligned trace arrays with shape (2, n),
              where the first row is time and the second row is current.
    ...
    resampled_current = [series_resampler.fit_transform(series.iloc[:, 1].to_numpy().T).ravel()
    ...
    for i in range(len(resampled_data) - 1):
        if i != min_index:
            ...
            if constraint is not None:
                path, _ = dtw_path(series, series_standard, global_constraint="sakoe_chiba",
    ...
    # align_data_after_dtw-align with the longest data
    ...
    interplotation_aligned_data = [resampler.fit_transform(series[-1, :]).ravel()
                                   for series in aligned_data_unique]
